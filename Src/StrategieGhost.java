...
public class StrategieGhost implements Strategie {
  ...
  { 
    ...
    while(!liPos.isEmpty()){
      ...
      if(nouvPos.equals(ghost.getPosition())){
        // vu qu'on a testé de 0 à 4 les diréctions de la racine, alors celle qui est la plus élevée est la dernière testée, donc la plus proche.
        if(!labyrinthe.isWall(ghost.getPosition().getX(), ghost.getPosition().getY()+1) && Chemin.contains(nextPosition(disponibles, ghost.getPosition() , (byte)1))) 
          move=1;
        if(!labyrinthe.isWall(ghost.getPosition().getX()+1, ghost.getPosition().getY()) && Chemin.contains(nextPosition(disponibles, ghost.getPosition() , (byte)2))) 
          move=2;
        if(!labyrinthe.isWall(ghost.getPosition().getX()-1, ghost.getPosition().getY()) && Chemin.contains(nextPosition(disponibles, ghost.getPosition() , (byte)3))) 
          move=3;
        return move;
      }
      
      // ajout des voisins de nouvPos qui n'ont pas déjà été traités
      if(!labyrinthe.isWall(nouvPos.getX(), nouvPos.getY()-1) && !Chemin.contains(nextPosition(disponibles, nouvPos , (byte)0))) 
        liPos.add(nextPosition(disponibles, nouvPos , (byte)0));
      if(!labyrinthe.isWall(nouvPos.getX(), nouvPos.getY()+1) && !Chemin.contains(nextPosition(disponibles, nouvPos , (byte)1))) 
        liPos.add(nextPosition(disponibles, nouvPos , (byte)1));
      if(!labyrinthe.isWall(nouvPos.getX()+1, nouvPos.getY()) && !Chemin.contains(nextPosition(disponibles, nouvPos , (byte)2))) 
        liPos.add(nextPosition(disponibles, nouvPos , (byte)2));
      if(!labyrinthe.isWall(nouvPos.getX()-1, nouvPos.getY()) && !Chemin.contains(nextPosition(disponibles, nouvPos , (byte)3))) 
  ...
  private PositionAgent nextPosition(ArrayList<PositionAgent> freePos, PositionAgent currentPos, byte direction){
    ...
    switch(direction){
      case(0):{ // north
        ...
        }while(Math.abs(result.getY())!=10000);
       break; 
      }
      case(1):{ // south
        do{
      ...
      case(2):{ // east
        do{
          ...
          if(freePos.contains(result)) break;
        }while(Math.abs(result.getX())!=10000);
       break; 
      } 
      case(3):{ // west
        do{
